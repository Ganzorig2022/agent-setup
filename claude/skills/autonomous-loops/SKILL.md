...
## Bundled references (merged in)
Folded from former standalone skills during consolidation:
- `reference-loop-selection.md` — loop-type selection flow (sequential / continuous-pr / rfc-dag / infinite), combined production stack, failure modes, and recovery steps. (was `continuous-agent-loop`)
- `reference-verification.md` — 6-phase verification system (build → types → lint → tests/coverage → security scan → diff review) with a READY/NOT-READY report format and continuous-mode checkpoints. (was `verification-loop`)

Read these when you need loop-type selection or an end-of-iteration quality gate.
