...
- `reference-cost-pipeline.md` — cost-aware LLM pipeline patterns: model routing by task complexity, immutable cost tracking, narrow retry logic, and prompt caching. (was `cost-aware-llm-pipeline`)

Read this when building/optimizing an app that calls LLM APIs (vs. tracking your own Claude Code spend, which is this skill's primary job).
