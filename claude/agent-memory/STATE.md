...
- Codex mirrors Claude: ~/.codex/{agents/*.toml, skills/, rules/}. Claude files (~/.claude/agents/*.md) are source of truth; Codex .toml are ports. Both tools have their own native reviewer agents — Claude need not review Codex's changes or vice versa.
