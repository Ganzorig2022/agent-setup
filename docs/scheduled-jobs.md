...
| `review-intel-harvest.py` | manual Stage 0A inspection | Local-only, zero-model reviewer-trace harvest into `~/.review-intelligence/`; analysis and scheduling stay disabled until privacy, parse-rate, usable-finding, and source-identity gates pass. Usage logs are cumulative telemetry, not a run ledger |
