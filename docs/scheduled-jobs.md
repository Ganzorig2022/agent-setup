...
| **review-intelligence-harvest**<br>`com.dev.review-intelligence-harvest` | daily 02:30 + login catch-up | `review-intel-harvest.py` | Idempotently preserves redacted reviewer run/finding records before Claude's 30-day transcript expiry. Harvest only: no analysis, model calls, network, prompt changes, or delivery | Nothing | Fully local; retries at next login/calendar run. Store is intentionally not backed up; machine loss restarts the 20-run gate | `~/.review-intelligence/{traces,collection}.jsonl` + latest `summary.json` | None |
