...
| **tech-brief**<br>`com.dev.tech-brief` | daily 08:00 + login catch-up | `tech-brief.py` | RSS + X harvest → Claude-curated daily AI/dev brief; drafts X replies (auto-post via `x-reply.py` currently broken — see below) | Everything (feeds, X, Claude, Telegram) | Retries whole fetch every 15 min up to **20h**; skips if today's brief <4h old | `~/tech-brief/<date>.md` (symlinked onto Desktop — same TCC reason as x-deepdive; never write the Desktop subtree directly) | Telegram (outbox on failure); failed X replies degrade to manual drafts in the brief |
...
| `x-reply.py` | called by tech-brief | Posts drafted X replies via logged-in headless Chrome; safety rails: kill switch `~/Desktop/tech-brief/.no-auto-reply`, dedupe cache, 4/run cap, jittered spacing. **KNOWN BROKEN (2026-07-05):** posting fails at the inline Reply-button/`verify_posted` step (`Reply button not found/clickable`, 0/N posted) while the read session stays healthy — replies degrade to manual drafts in the brief. Needs a supervised chrome-devtools-axi fix (dry-run first). Cosmetic: `@@handle` double-`@` in logs |
